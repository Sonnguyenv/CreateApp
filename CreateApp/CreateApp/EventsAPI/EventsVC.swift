...
class EventsVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let json = try JSONDecoder().decode(EventAPI.self, from: data)
                let dataEvents = json.response?.events
                let dataVanue = json.response?.events?.venue
                let dataCategory = json.response?.events?.category
                print(dataCategory?.name ?? "")
                let imageUrl = URL(string: json.response?.events?.photo ?? "")
